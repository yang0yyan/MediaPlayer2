...
#define  LOG_TAG  "media_player-lib"
...
public:
    int readFile(JNIEnv *env, const char *url, jobject activity);
    void release();


    int initVideo(JNIEnv *env, int width, int height, int rotation);
    int setSurface(JNIEnv *env, jobject surface);
    int removeSurface();
    int videoWrite(uint8_t *src, int srcStride);
