...
int MediaPlayer::readFile(JNIEnv *env, const char *url, jobject activity) {
    initData();
...
int MediaPlayer::initVideo(JNIEnv *env, int width, int height, int rotation) {
    videoChannel.init(env, width, height, rotation);
    return 0;
}

int MediaPlayer::setSurface(JNIEnv *env, jobject surface) {
    videoChannel.setSurface(env, surface);
    return 0;
}

int MediaPlayer::removeSurface() {
    videoChannel.removeSurface();
    return 0;
}

int MediaPlayer::videoWrite(uint8_t *src, int srcStride) {
    videoChannel.write(src,srcStride);
    return 0;
}
///------------------------------------------------------------------------------
